...
function [x_est, P_est] = E1(odo, zind, z, V, W, x0, P0, map)
    ...
    for k = 1:steps
        ...
        if z_i ~= 0
            ...
            H_x = Hx(x_pred(1), x_pred(2), landmark(1), landmark(2), r);
            H_w = Hw();
...
function x_est = dynamics(x_k, y_k, theta_k, odo_d, odo_theta, v_d, v_theta)
    x_est = [...
        x_k+(odo_d+v_d)*cos(theta_k);...
        y_k+(odo_d+v_d)*sin(theta_k);...
        theta_k+odo_theta+v_theta;...
    ];
end

function F_x = Fx(odo_d, theta_v)
    F_x = [...
