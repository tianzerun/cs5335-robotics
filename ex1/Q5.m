...
function q = Q5(f1, f2, qInit, f1Target, f2Target)    
    ...
    while norm(dx) > epsilon
        f1_from = q(1:9);
        f2_from = [q(1:7) q(10:11)];
        
        f1_x = f1.fkine(f1_from).t;
        f2_x = f2.fkine(f2_from).t;
        
        f1_dx = f1Target - f1_x;
        f2_dx = f2Target - f2_x;
        dx = [f1_dx;0;0;0;f2_dx;0;0;0];
        
        f1_j = [f1.jacob0(f1_from) zeros(6,2)];
        f2a_j = f2.jacob0(f2_from);
        f2_j = [f2a_j(:,1:7) zeros(6, 2) f2a_j(:,8:9)];
        invJ = [f1_j ; f2_j]';
